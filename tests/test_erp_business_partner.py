...
class ErpBusinessPartnerTests(unittest.TestCase):
    ...
    def test_index_contains_erp_business_partner_flow(self):
        ...
        self.assertIn("customerProfileLoadingPlaceholder = '<Đang tải...>'", html)
        self.assertIn("customerProfileConflictModal", html)
        self.assertIn("currentCode !== requestedLookupKey", html)
        self.assertIn("applyCustomerLocalProfile(profile, customerCode, lookupSession);", html)
        self.assertIn("applyErpBusinessPartnerProfile(profile, customerCode, lookupSession);", html)
        self.assertIn("applyPurchaseOrderCustomerProfile(profile, customerCode, lookupSession);", html)
        self.assertIn("const canReplaceUntouched = !state.userDirty && currentNormalized === state.lastProgrammaticValue;", html)
        self.assertIn("queueCustomerProfileConflictPrompt(field, currentValue, value, options.lookupKey, options.lookupSession);", html)
        self.assertIn("if (!event || !event.isTrusted) return;", html)
        self.assertIn("if (currentNormalized === prompt.expectedCurrentNormalized)", html)
        self.assertIn("resetCustomerProfileAutofillState(false);", html)
        self.assertIn("resetCustomerLookupState({ preserveCustomerCode: !!key });", html)
        self.assertIn("resetCustomerLookupState({ preserveCustomerCode: false });", html)
        self.assertIn("isCurrentCustomerProfileLookupSession(lookupSession, customerCode)", html)
        self.assertIn("clearCustomerProfileLoadingPlaceholdersForLookup(customerCode, lookupSession);", html)
        self.assertIn("applyCustomerProfileLoadingPlaceholders(profile, customerCode, lookupSession);", html)
        self.assertIn("clearCustomerInfoFields({ preserveCustomerCode: !!key });", html)
        self.assertIn("btnClearCustomerBlock", html)
        self.assertNotIn("btnOcrClip", html)
