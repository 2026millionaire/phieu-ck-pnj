...
class CustomerUpdateApiTests(unittest.TestCase):
    ...
    def test_settings_and_default_sap_state_follow_server_role(self):
        ...
        self.assertIn('id="btnClearCustomerBlock"', user_html)
        self.assertIn("Xoá hết", user_html)
        ...
        self.assertNotIn('id="btnOcrClip"', admin_html)
        self.assertNotIn("appUrl('/api/ocr-bk')", admin_html)
        self.assertIn('id="btnClearCustomerBlock"', admin_html)
        self.assertIn("clearCustomerInfoFields({ preserveCustomerCode: false });", admin_html)
        self.assertIn("resetCustomerLookupState({ preserveCustomerCode: false });", admin_html)
    ...
    def test_thudoi_link_is_admin_only(self):
        self.login(role="user", user_id=2)
        user_html = self.client.get("/").get_data(as_text=True)
        self.assertNotIn("Chế độ thu đổi", user_html)
        self.assertNotIn('href="/thudoi/"', user_html)

        self.login(role="admin", user_id=1)
        admin_html = self.client.get("/").get_data(as_text=True)
        self.assertIn("Chế độ thu đổi", admin_html)
        self.assertIn('href="/thudoi/"', admin_html)
