...
class EofficeQt82Tests(unittest.TestCase):
    ...
    def test_payment_planning_output_and_bk_save_copy_flow(self):
        ...
        self.assertIn("margin: 7mm 10.5mm 7mm 25mm", html)
        ...
        self.assertNotIn('<div class="section-title">VI. Nguyên tắc thanh toán</div>', page_one_html)
        ...
        self.assertIn('<span class="checkbox-mark">&#9745;</span> Chuyển khoản', page_one_html)
        self.assertIn('<span class="checkbox-mark">&#9744;</span> Khác', page_one_html)
        self.assertIn('<col style="width: 15%;">', html)
        self.assertIn('<col style="width: 17.5%;">', html)
        self.assertIn('<col style="width: 21.5%;">', html)
        self.assertIn("grid-template-rows: 38px 42px 20px;", html)
        self.assertIn("height: 104px;", html)
        ...
        self.assertIn("line-height: 1.1", colored_html)
    ...
    def test_bk_create_and_history_have_payment_planning_actions(self):
        ...
        self.assertIn('id="previewPaymentSchedule"', index_html)
        self.assertIn("1. T/T+1: 10% - tương ứng số tiền", index_html)
        self.assertIn("5. T+120: 20% - tương ứng số tiền", index_html)
        self.assertIn('id="previewPaymentNote"', index_html)
        self.assertIn("0234 3847 588", index_html)
        self.assertNotIn('id="previewPaymentTimeText"', index_html)
        ...
        self.assertIn("applyPurchaseOrderCustomerProfile(profile, customerCode, lookupSession);", index_html)
        self.assertIn("customerProfileConflictModal", index_html)
        self.assertIn("queueCustomerProfileConflictPrompt(field, currentValue, value, options.lookupKey, options.lookupSession);", index_html)
        self.assertIn("if (!event || !event.isTrusted) return;", index_html)
        self.assertIn("resetCustomerLookupState({ preserveCustomerCode: !!key });", index_html)
        self.assertIn("resetCustomerLookupState({ preserveCustomerCode: false });", index_html)
        self.assertIn('id="btnClearCustomerBlock"', index_html)
        self.assertIn("Xoá hết", index_html)
        self.assertNotIn("btnOcrClip", index_html)
        ...
        self.assertIn("autoFillPurchaseOrderSuggestions", index_html)
        ...
        self.assertIn("<Đang tải BN cọc...>", index_html)
        self.assertIn("/api/deposit-suggestions", index_html)
        self.assertIn("transactionAutoLoadChoiceModal", index_html)
        self.assertIn("handleTransactionManualIntent", index_html)
        self.assertIn("<Đang tải...>", index_html)
    ...
    def test_transaction_autoload_sap_note_waits_for_completion_and_uses_single_bk_gate(self):
        ...
        self.assertIn("customerProfileLoadingPlaceholder = '<Đang tải...>'", template_text)
        self.assertIn("applyCustomerProfileLoadingPlaceholders(profile, customerCode, lookupSession);", template_text)
        self.assertIn("clearCustomerProfileLoadingPlaceholdersForLookup(customerCode, lookupSession);", template_text)
        self.assertIn("if (currentNormalized === prompt.expectedCurrentNormalized)", template_text)
        self.assertIn("clearCustomerInfoFields({ preserveCustomerCode: !!key });", template_text)
    ...
    def test_source_includes_sap_document_in_save_payload_without_creating_transaction_row(self):
        ...
        self.assertIn("cccd: document.getElementById('cccd').value.trim()", template_text)
        self.assertIn("dia_chi: document.getElementById('dia_chi').value.trim()", template_text)
